...
export const parameterListTemplate = (jsonData) => {
  ...
  if (
    dataSummaryVizChart2.dataset.selectedVariable &&
    dataSummaryVizChart2.dataset.selectedVariable !== ""
  ) {
    selectedVariable = dataSummaryVizChart2.dataset.selectedVariable;
  }
  if (!jsonData) {
    document.getElementById("variableLabel").innerHTML =
      "Most useful variables";
    document.getElementById("parameterList").classList.remove("list-variables");
    if (document.getElementById("toggleVariable")) {
      document.getElementById("toggleVariable").innerHTML =
        'Show All <i class="fas fa-caret-down"></i>';
    } else {
      document.getElementById("showAllVariables").innerHTML =
        '<a href="#" id="toggleVariable">Show All <i class="fas fa-caret-down"></i></a>';
    }
    parameters = ["ER_statusIndex", "ethnicityClass", "famHist", "contrType"];
    parameters.forEach((param) => {
      template += `<a class="list-group-item variableItem ${
        param === selectedVariable ? `active` : ``
      }" href="#">${param}</a>`;
    });
  } else {
    ...
    parameters = Object.keys(
      jsonData[parametersLength.indexOf(maximumparameters)]
    );
    parameters.sort();
    ...
    if (document.getElementById("toggleVariable")) {
      ...
      document.getElementById("showAllVariables").innerHTML =
        '<a href="#" id="toggleVariable">Show Less <i class="fas fa-caret-up"></i></a>';
    }

    parameters.forEach((param) => {
      if (config.invalidVariables.indexOf(param.trim().toLowerCase()) !== -1)
        return;
      template += `<a class="list-group-item variableItem ${
        param === selectedVariable ? `active` : ``
      }" href="#">${param}</a>`;
