...
export const loginAppDev = () => {
    ...
    location.href=`https://account.box.com/api/oauth2/authorize?response_type=code&client_id=${config.iniAppLocal.client_id}&redirect_uri=${location.origin}&state=${config.iniAppLocal.stateIni}`
}

export const loginAppEpisphere = () => { 
    console.log(config.iniAppDev.stateIni);
    location.href=`https://account.box.com/api/oauth2/authorize?response_type=code&client_id=${config.iniAppDev.client_id}&redirect_uri=${location.origin}&state=${config.iniAppDev.stateIni}`;
}
